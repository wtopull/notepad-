...
class TableBuilder
{
     ...
     * @param string $ 提示类型：danger,info,warning,success
     * @param string $pos  提示位置：top,search,bottom
     * @return $this
     */
    public function setPageTips($tips = '', $ = 'info', $pos = 'top')
    {
        if ($tips != '') {
            $this->_vars['page_tips_' . $pos] = $tips;
            $this->_vars['tips_'] = trim($) ?? 'info';
        }
        return $this;
    }
     ...
     * @return $this
     */
    public function addColumn($name = '', $title = '', $ = '', $default = '', $param = '', $class = 't_c')
    {
        $column = [
            'name'    => $name,
            'title'   => $title,
    ...
    /**
     * 添加一个右侧按钮
     * @param string $       按钮类型：edit/delete/default
     * @param array $attribute   按钮属性
     * @param array $extra       扩展参数(待用)
     * @return $this
     */
    public function addRightButton($ = '', $attribute = [])
    {
        switch ($) {
            // 编辑按钮
            case 'edit':
